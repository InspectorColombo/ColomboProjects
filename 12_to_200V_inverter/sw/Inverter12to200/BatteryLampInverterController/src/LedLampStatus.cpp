...
namespace LedLamp
{
static const uint16_t ST_OVERTEMPERATURE			= 0b00000001;
static const uint16_t ST_CONVERTER_ON				= 0b00000010;
static const uint16_t ST_LOW_BATTERY				= 0b00000100;
...
bool LedLampStatus::IsAddVoltageON() const
{
	return (m_statusByte & ST_ADD_VOLTAGE) != 0;
}
void LedLampStatus::SetAddVoltage(const bool nv)
{
	m_statusByte = nv ? (m_statusByte | ST_ADD_VOLTAGE) : (m_statusByte & (~ST_ADD_VOLTAGE));
}

bool LedLampStatus::IsChargeFinishON() const
{
	return (m_statusByte & ST_CHARGE_FINISH) != 0;
}
void LedLampStatus::SetChargeFinish(const bool nv)
{
